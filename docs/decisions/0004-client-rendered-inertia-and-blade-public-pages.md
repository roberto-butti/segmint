...
server-rendered without introducing an SSR runtime for the application. Public pages may
use progressively enhanced JavaScript where needed, but they must not depend on the
Inertia application shell.

The current `/` homepage remains an Inertia page until it is replaced by the public Blade
homepage. It is a temporary exception, not the pattern for new public pages.
